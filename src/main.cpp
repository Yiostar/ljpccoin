...
#if defined(NDEBUG)
# error "LJpccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc4096da56e1211ed6c6d2da2fe828cccaddf20c397cbdf293812d1088b6831a9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LJpccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LJpccoin Signed Message:\n";
...
{
    // LJpccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // LJpccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // LJpccoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // LJpccoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LJpccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LJpccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbf764d77c06455faed4e5858ca9b4fa6ca77a976f0315319c4ffad053eedf2a3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Wild moose seen in Denmark for the first time in 20 years";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041c8910b51f6e4069e47f6f8c4ed1c2495ef06916fd296e3ad99c597508e04ba127c84e7a27896ecdc9237dae9368172f56dee697350b5058b9326616df223fb9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1529509215;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084987443;

        if (fTestNet)
        {
            block.nTime    = 1529509174;
            block.nNonce   = 386941140;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x82b61ee3f56b9043fe3c79adeef778a7511d848b14cc8fec3514c5f0fb18c969"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xb8, 0xde }; // LJpccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LJpccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LJpccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LJpccoinMiner(CWallet *pwallet)
{
    printf("LJpccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ljpccoin-miner");
    ...
    try { loop {
        ...
        printf("Running LJpccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LJpccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LJpccoinMiner, pwallet));
}
